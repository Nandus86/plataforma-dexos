 ...
 *
 * Estrategia: maquina de estados no lado RECEIVE apenas.
 * Padrao do binario Hikvision em cada loop de deteccao:
 *   1. GETLINK -> recebe interfaces/MACs (RTM_NEWLINK multi-part + DONE)
 *   2. GETADDR -> recebe IPs (em Docker: so IPv6 ou vazio + DONE)
 * Interceptamos no passo 2 para injetar IPv4.
 */
#define _GNU_SOURCE
#include <dlfcn.h>
...
/* Fases da maquina de estados (por socket) */
#define PHASE_IDLE           0
#define PHASE_SAW_NEWLINK    1  /* Viu dados RTM_NEWLINK, aguardando DONE */
#define PHASE_EXPECT_GETADDR 2  /* DONE do GETLINK recebido, proximo e GETADDR */
...
/* ======== build_response: criar resposta IPv4 falsa ======== */
static ssize_t build_response(void *buf, size_t buflen,
    ...
    for (ifa = ifas; ifa; ifa = ifa->ifa_next) {
        ...
        size_t addr_data = NLMSG_ALIGN(sizeof(struct ifaddrmsg))
                         + RTA_SPACE(4) + RTA_SPACE(4) + RTA_SPACE(4);
        ...
        rta = (struct rtattr *)ap;
        rta->rta_type = IFA_ADDRESS; rta->rta_len = RTA_LENGTH(4);
        ...
        rta = (struct rtattr *)ap;
        rta->rta_type = IFA_BROADCAST; rta->rta_len = RTA_LENGTH(4);
...
/* Drenar mensagens restantes do GETADDR ate encontrar DONE */
...
ssize_t recvfrom(int fd, void *buf, size_t len, int flags,
    ...
    case PHASE_IDLE:
        if (nlh->nlmsg_type == RTM_NEWLINK) {
            state->phase = PHASE_SAW_NEWLINK;
        }
        break;

    case PHASE_SAW_NEWLINK:
        if (nlh->nlmsg_type == NLMSG_DONE) {
            /* Fim do GETLINK. Proximo e resposta do GETADDR. */
            state->phase = PHASE_EXPECT_GETADDR;
        } else if (nlh->nlmsg_type != RTM_NEWLINK) {
            state->phase = PHASE_IDLE;
        }
        /* Se ainda RTM_NEWLINK, ficar em SAW_NEWLINK */
        break;

    case PHASE_EXPECT_GETADDR:
        if (nlh->nlmsg_type == NLMSG_DONE && result == 20) {
            /* GETADDR vazio (Docker sem IPs visivel ao netlink) */
            state->phase = PHASE_IDLE;
            ...
            if (ifa->ifa_family == AF_INET6) {
                /* Docker retornou IPv6. Precisamos:
                 * 1. Drenar mensagens IPv6 restantes + DONE real
                 * 2. Injetar nosso IPv4 no lugar */
                uint32_t s = nlh->nlmsg_seq, p = nlh->nlmsg_pid;

                /* Verificar se DONE ja esta neste buffer */
                int found_done = 0;
                ...
                /* Se DONE nao esta neste buffer, drenar ate encontrar */
                if (!found_done) {
                    drain_until_done(fd, buf, len);
                }
                ...
                /* Docker retornou IPv4 real! Nao precisamos injetar. */
                state->phase = PHASE_IDLE;
            }
            else {
                state->phase = PHASE_IDLE;
